...
from src.data.transform import Compose, Resize, Pad, Normalize, OutData, RandomHorizontalFlip, RandomResize
...
def preprocess_fn(args, image_id, image, image_anno_dict, is_training):
    ...
    if is_training:
        ...
        trans = Compose([
            ...
            RandomResize(max_h_arr, args.max_size),
            Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]),
        ])
        out_data = OutData(is_training=True, max_size=args.max_size)
    else:
        trans = Compose([
            Resize(args.max_size),
            Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]),
        ])
        out_data = OutData(is_training=False, max_size=args.max_size)
